...
# =====================================================
class mamba_init:
    # 用于初始化 dt_proj（一个线性变换层）和其偏置项 dt_proj.bias。保证在初始化时，dt_proj 的权重和偏置项符合特定的分布。
    @staticmethod
    def dt_init(dt_rank, d_inner, dt_scale = 1.0, dt_init = "random", dt_min = 0.001, dt_max = 0.1,
        ...
        dt_init_std = dt_rank ** -0.5 * dt_scale  # 标准差
        if dt_init == "constant":  # 常量
            nn.init.constant_(dt_proj.weight, dt_init_std)
        elif dt_init == "random":  # 均匀分布
            nn.init.uniform_(dt_proj.weight, -dt_init_std, dt_init_std)
        else:
            raise NotImplementedError

        # Initialize dt bias so that F.softplus(dt_bias) is between dt_min and dt_max
        # 使用 softplus 函数的反函数来初始化，使得偏置项的值落在 [dt_min, dt_max] 之间。
        # 使用 torch.exp 和 torch.log 来确保初始化值满足所需范围，并避免过小的偏置值（通过 dt_init_floor）。
        dt = torch.exp(
        ...
        # 返回 dt_proj 包含初始化的权重和偏置
        return dt_proj

    # 初始化参数 A_log，它是一个对数值，通常用于表示状态空间或在某些公式中作为缩放因子。
    @staticmethod
    def A_log_init(d_state, d_inner, copies = -1, device = None, merge = True):
        # S4D real initialization
        # 生成一个从 1 到 d_state 的序列，转换为 float32 类型，并重复 d_inner 次，形成一个形状为 (d_inner, d_state) 的矩阵
        A = torch.arange(1, d_state + 1, dtype = torch.float32, device = device).view(1, -1).repeat(d_inner,
                                                                                                    1).contiguous()
        A_log = torch.log(A)  # Keep A_log in fp32
        if copies > 0:
            A_log = A_log[None].repeat(copies, 1, 1).contiguous()  # 维度[copies, m, n]
            if merge:
                A_log = A_log.flatten(0, 1)  # 将维度展平为[copies * m, n]
        A_log = nn.Parameter(A_log)
        A_log._no_weight_decay = True
        # 返回初始化的 A_log 参数，它是一个 nn.Parameter 对象，并标记为不参与权重衰减
        return A_log

    # 初始化参数 D，通常是一个“跳跃”参数，可能用于某些残差连接或缩放因子。
    @staticmethod
    def D_init(d_inner, copies = -1, device = None, merge = True):
        # D "skip" parameter
        D = torch.ones(d_inner, device = device)
        if copies > 0:
            D = D[None].repeat(copies, 1).contiguous()  # 维度[copies, m, n]
            if merge:
                D = D.flatten(0, 1)  # 将维度展平为[copies * m, n]
        D = nn.Parameter(D)  # Keep in fp32
        D._no_weight_decay = True
        # 返回初始化的 D 参数，它是一个 nn.Parameter 对象，并标记为不参与权重衰减
        return D
...
class SS2Dv2:
    ...
    def forward_corev2(
        ...
        _scan_mode = dict(cross2d = 0, unidi = 1, bidi = 2, cascade2d = -1).get(scan_mode, None) if isinstance(
...
class SS2Dv3:
    ...
    def forwardxv(self, x: torch.Tensor, **kwargs):
        ...
        us = cross_scan_fn(us.contiguous(), in_channel_first = self.channel_first, out_channel_first = True).view(B, -1,
                                                                                                                  L)
...
class Backbone_VSSM(VSSM):
    def __init__(self, out_indices = (0, 1, 2, 3), pretrained = None, norm_layer = "ln", **kwargs):
        ...
        del self.classifier
        # pretrained = './vmamba/vmambaweight/vssm_small_0229_ckpt_epoch_222.pth'
    ...
    # def load_pretrained_ema(model, model_ema):
    #     # logger.info(f"==============> Loading weight {config.MODEL.PRETRAINED} for fine-tuning......")
    #     print('loading pretrained weights!')
    #     weights_path = './vmamba/vmambaweight/vssm_small_0229_ckpt_epoch_222.pth'
    ...
    #         print(msg)
    #         print(f"=> loaded 'model' successfully from '{weights_path}' !")
    #     else:
    #         # logger.warning(f"No 'model' found in {config.MODEL.PRETRAINED}! ")
    #         print(f"No 'model' found in {weights_path} !")
    #
    ...
    #             print(msg)
    #             print(f"=> loaded 'model' successfully from '{weights_path}'for model_ema !")
    #         else:
    #             # logger.warning(f"No '{key}' found in {config.MODEL.PRETRAINED}! ")
    #             print(f"No '{key}' found in {weights_path}! ")
    #
    #     del checkpoint
    #     torch.cuda.empty_cache()
